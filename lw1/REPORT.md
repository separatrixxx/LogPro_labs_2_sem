...
Реализация метода `pop();` из JS на прологе
Предикат получает на вход список и две переменные. Удаляет последний элемент списка и возвращет его. Также выводит получившийся список
