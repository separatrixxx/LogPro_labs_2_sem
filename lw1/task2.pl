...
% Вариант 3, представление one.pl
...
% Находим количество оценок 2 за каждый предмет
sbj_fail(SbjAbbr, N) :-
...
% Удаляем первые 6 элементов
del_first(L,R):- 
    append([_, _, _, _, _, _],R,L).

% Список из сумм оценок, сортируем, реверсируем и получаем самую большую сумму
